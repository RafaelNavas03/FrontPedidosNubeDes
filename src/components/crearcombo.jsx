...
const NuevoComboForm = () => {
    ...
    const fetchProductos = async (page = 1, size = 8, search = '') => {
        try {
            const response = await fetch(`https://gestorpedidosbaknube-production.up.railway.app/producto/listar/?page=${page}&size=${size}&search=${search}`);
            if (response.ok) {
    ...
    const fetchCategorias = async () => {
        try {
            const response = await fetch('https://gestorpedidosbaknube-production.up.railway.app/combos/listcategoria/');
            if (response.ok) {
                const data = await response.json();
    ...
    const onFinishProductos = async () => {
        try {
            ...
            const response = await fetch('https://gestorpedidosbaknube-production.up.railway.app/combos/crearcombo/', {
                method: 'POST',
                body: formData,
            });
    ...
    const onFinishCategoria = async (values) => {
        try {
            ...
            const response = await fetch('https://gestorpedidosbaknube-production.up.railway.app/combos/crearcat/', {
                method: 'POST',
                body: formData,
            });
                ...
                <Form form={categoriaCombo} name="categoriaCombo" onFinish={onFinishCategoria} labelCol={{ span: 8 }} wrapperCol={{ span: 16 }}>
                    <Form.Item name="catnombre" label="Nombre"
                        ...
                        { max: 300, message: 'El nombre de la categoría no puede tener más de 300 caracteres' },
                        {
                            validator: async (_, value) => {
                                try {
                                    const response = await fetch('https://gestorpedidosbaknube-production.up.railway.app/combos/categoriaExist/', {
                                        method: 'POST',
                                        headers: {

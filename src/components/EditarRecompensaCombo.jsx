...
const EditarRecompensaComboForm = () => {
  ...
  useEffect(() => {
    const fetchRecompensasCombos = async () => {
      try {
        const response = await fetch('https://gestorpedidosbaknube-production.up.railway.app/Recompensas/lista_recompensas_combo/');
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchCombos = async () => {
      try {
        const response = await fetch('https://gestorpedidosbaknube-production.up.railway.app/combos/ver_combos/');
        if (!response.ok) {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch(`https://gestorpedidosbaknube-production.up.railway.app/Recompensas/editar_recompensa_combo/${editedRecompensaCombo.id_recompensa_combo}/`, {
        method: 'POST',
        body: formData,
      });

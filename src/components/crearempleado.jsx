...
const CrearEmpleadoForm = () => {
  ...
  const onFinish = async (values) => {
    try {
      const response = await fetch('https://gestorpedidosbaknube-production.up.railway.app/empleado/crear/', {
        method: 'POST',
        headers: {
        ...
        rules={[
          ...
          { max: 300, message: 'El nombre de usuario no puede tener más de 300 caracteres' },
          {
            validator: async (_, value) => {
              try {
                const response = await fetch('https://gestorpedidosbaknube-production.up.railway.app/Login/cuentaexist/', {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                  },
      ...
      <Form.Item
        ...
        rules={[
          ...
          { max: 10, message: 'El número de teléfono no puede tener más de 10 caracteres' },
          {
            validator: async (_, value) => {
              try {
                const response = await fetch('https://gestorpedidosbaknube-production.up.railway.app/Login/phoneExist/', {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                  },

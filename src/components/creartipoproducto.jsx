...
const CrearTipoProducto = () => {
  ...
  const onFinish = async (values) => {
    setLoading(true);
    try {
      const response = await fetch('https://gestorpedidosbaknube-production.up.railway.app/producto/creartipop/', {
        method: 'POST',
        headers: {
        ...
        rules={[
          { required: true, message: 'Por favor ingresa el nombre del tipo de producto' },
          {
            validator: async (_, value) => {
              try {
                const response = await fetch('https://gestorpedidosbaknube-production.up.railway.app/producto/tipoProductoExist/', {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                  },
